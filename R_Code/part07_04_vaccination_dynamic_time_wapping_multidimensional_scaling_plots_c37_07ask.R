...
# vaccination
load(file = paste("../R_Data/pairs_frame_vaccination_tw.RData"))
ls()
...
# vaccination
tw_distance_matrix_vaccination_c37 = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(tw_distance_matrix_vaccination_c37) = countries
rownames(tw_distance_matrix_vaccination_c37) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    row_index = which((pairs_frame_vaccination_tw$Country1 == current_country1) * (pairs_frame_vaccination_tw$Country2 == current_country2) +
      (pairs_frame_vaccination_tw$Country1 == current_country2) * (pairs_frame_vaccination_tw$Country2 == current_country1) == 1)

    # Filling the value
    if (current_country1 != current_country2) {
      tw_distance_matrix_vaccination_c37[current_country1, current_country2] = pairs_frame_vaccination_tw$tw_statistic[row_index]
    }
...
tw_distance_matrix_vaccination_c37 <- tw_distance_matrix_vaccination_c37/max(tw_distance_matrix_vaccination_c37) * 1000
# Exporting 
tw_distance_matrix_vaccination_c37_csv_path = paste("../R_Output/tw_distance_matrix_vaccination_c37.csv", sep = "")
write.table(x    = data.frame( country = gsub(pattern = " ", replacement = ".", x = rownames(tw_distance_matrix_vaccination_c37)), tw_distance_matrix_vaccination_c37),
            file = tw_distance_matrix_vaccination_c37_csv_path, sep = ",", row.names = FALSE)




# vaccination
tw_distance_points_vaccination = cmdscale(tw_distance_matrix_vaccination_c37)
x_values_vaccination = tw_distance_points_vaccination[, 1]
y_values_vaccination = tw_distance_points_vaccination[, 2]
# reflect so North is at the top
## note asp = 1, to ensure Euclidean distances are represented correctly


# Generating pdf output.
pdf(paste("../Plots/part07_04_vaccination_dynamic_time_wapping_vaccination_c37", graph_suffix, ".pdf", sep = ""), height = 16, width = 16)
...
plot(
  ...
  main = "vaccination (Dynamic Time Wrapping Distance)",
...
text(x = x_values_vaccination, y = y_values_vaccination, labels = rownames(tw_distance_points_vaccination), srt=45,  cex = 1.3)
dev.off()
...
for (method_current in hclust_methods)
{
  ...
  # Computing clusters
  cluster_vaccination = hclustfunc(tw_distance_matrix_vaccination_c37, method_selected = method_current)
  
  
  # Fix 2022.11.28 
  
  # Creating object
  create_object_command <- paste0( "dynamic_time_wapping_hclust_vaccination_c37_", method_current, " <- cluster_vaccination" )
  eval(parse(text = create_object_command))    
  
  # Saving object
  # Path to save
  path_to_save_object <- paste0("../R_Data/dynamic_time_wapping_hclust_vaccination_c37_", method_current, ".RData") 
  # Saving process
  saving_process_command <- paste0( "save( dynamic_time_wapping_hclust_vaccination_c37_", method_current, ", file = path_to_save_object )" )
  eval(parse(text = saving_process_command))    
  
  
  
  # Generating pdf output.
  pdf(paste("../Plots/part07_04_vaccination_dynamic_time_wapping_clusters_vaccination_c37_", method_current, graph_suffix, ".pdf", sep = ""), height = 52, width = 40)
  # Defining the number of plots
  par(mfrow = c(1, 1))
  ...
  # Add main title
  mtext( paste0("Vaccination (Dynamic Time Wrapping Distance) - ", method_current),
    side = 3,
    line = -3.5,
    cex = 5,
    outer = TRUE
