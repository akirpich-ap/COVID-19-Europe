...
# vaccination
tw_distance_matrix_vaccination_c39 = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(tw_distance_matrix_vaccination_c39) = countries
rownames(tw_distance_matrix_vaccination_c39) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    if (current_country1 != current_country2) {
      tw_distance_matrix_vaccination_c39[current_country1, current_country2] = pairs_frame_vaccination_tw$tw_statistic[row_index]
    }
...
tw_distance_matrix_vaccination_c39 <- tw_distance_matrix_vaccination_c39/max(tw_distance_matrix_vaccination_c39) * 1000




# vaccination
tw_distance_points_vaccination = cmdscale(tw_distance_matrix_vaccination_c39)
...
pdf(paste("../Plots/part07_04_vaccination_dynamic_time_wapping_vaccination_c39", graph_suffix, ".pdf", sep = ""), height = 16, width = 16)
...
for (method_current in hclust_methods)
{
  ...
  # Computing clusters
  cluster_vaccination = hclustfunc(tw_distance_matrix_vaccination_c39, method_selected = method_current)
  
  
  # Fix 2022.11.28 
  
  # Creating object
  create_object_command <- paste0( "dynamic_time_wapping_hclust_vaccination_c39_", method_current, " <- cluster_vaccination" )
  eval(parse(text = create_object_command))    
  
  # Saving object
  # Path to save
  path_to_save_object <- paste0("../R_Data/dynamic_time_wapping_hclust_vaccination_c39_", method_current, ".RData") 
  # Saving process
  saving_process_command <- paste0( "save( dynamic_time_wapping_hclust_vaccination_c39_", method_current, ", file = path_to_save_object )" )
  eval(parse(text = saving_process_command))    
  
  
  
  # Generating pdf output.
  pdf(paste("../Plots/part07_04_vaccination_dynamic_time_wapping_clusters_vaccination_c39_", method_current, graph_suffix, ".pdf", sep = ""), height = 52, width = 40)
  # Defining the number of plots
  par(mfrow = c(1, 1))
