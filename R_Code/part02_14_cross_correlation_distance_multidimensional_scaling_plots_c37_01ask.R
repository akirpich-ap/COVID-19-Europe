...
# Loading cross-correlation statistics
# incidence
...
# incidence
cc_distance_matrix_incidence_c37 = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(cc_distance_matrix_incidence_c37) = countries
rownames(cc_distance_matrix_incidence_c37) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    if (current_country1 != current_country2) {
      cc_distance_matrix_incidence_c37[current_country1, current_country2] = pairs_frame_incidence_cc$cc_statistic[row_index]
    }
...
# Exporting 
cc_distance_matrix_incidence_c37_csv_path = paste("../R_Output/cc_distance_matrix_incidence_c37.csv", sep = "")
write.table(x    = data.frame( country = gsub(pattern = " ", replacement = ".", x = rownames(cc_distance_matrix_incidence_c37)), cc_distance_matrix_incidence_c37),
            file = cc_distance_matrix_incidence_c37_csv_path, sep = ",", row.names = FALSE)


# mortality
cc_distance_matrix_mortality_c37 = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(cc_distance_matrix_mortality_c37) = countries
rownames(cc_distance_matrix_mortality_c37) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    # Filling the value
    if (current_country1 != current_country2) {
      cc_distance_matrix_mortality_c37[current_country1, current_country2] = pairs_frame_mortality_cc$cc_statistic[row_index]
    }
...
# Exporting 
cc_distance_matrix_mortality_c37_csv_path = paste("../R_Output/cc_distance_matrix_mortality_c37.csv", sep = "")
write.table(x    = data.frame( country = gsub(pattern = " ", replacement = ".", x = rownames(cc_distance_matrix_mortality_c37)), cc_distance_matrix_mortality_c37),
            file = cc_distance_matrix_mortality_c37_csv_path, sep = ",", row.names = FALSE)




# incidence
cc_distance_points_incidence = cmdscale(cc_distance_matrix_incidence_c37)
...
pdf(paste("../Plots/part02_14_cross_correlation_distance_multidimensional_scaling_plots_incidence_c37", graph_suffix, ".pdf", sep = ""), height = 16, width = 16)
...
for (method_current in hclust_methods)
{
  ...
  # Creating object
  create_object_command <- paste0( "cross_correlation_distance_hclust_incidence_c37_", method_current, " <- cluster_incidence" )
  eval(parse(text = create_object_command))    
  
  # Saving object
  # Path to save
  path_to_save_object <- paste0("../R_Data/cross_correlation_distance_hclust_incidence_c37_", method_current, ".RData") 
  # Saving process
  saving_process_command <- paste0( "save( cross_correlation_distance_hclust_incidence_c37_", method_current, ", file = path_to_save_object )" )
  eval(parse(text = saving_process_command))    
  
  
  
  # Generating pdf output.
  pdf(paste("../Plots/part02_14_cross_correlation_distance_multidimensional_scaling_plots_clusters_incidence_c37_", method_current, graph_suffix, ".pdf", sep = ""), height = 52, width = 40)
  # Defining the number of plots
  par(mfrow = c(1, 1))
...
# mortality
cc_distance_points_mortality = cmdscale(cc_distance_matrix_mortality_c37)
...
# Generating pdf output.
pdf(paste("../Plots/part02_14_cross_correlation_distance_multidimensional_scaling_plots_mortality_c37", graph_suffix, ".pdf", sep = ""), height = 16, width = 16)
# Defining the number of plots
plot(
...
for (method_current in hclust_methods)
{
  ...
  # Creating object
  create_object_command <- paste0( "cross_correlation_distance_hclust_mortality_c37_", method_current, " <- cluster_mortality" )
  eval(parse(text = create_object_command))    
  
  # Saving object
  # Path to save
  path_to_save_object <- paste0("../R_Data/cross_correlation_distance_hclust_mortality_c37_", method_current, ".RData") 
  # Saving process
  saving_process_command <- paste0( "save( cross_correlation_distance_hclust_mortality_c37_", method_current, ", file = path_to_save_object )" )
  eval(parse(text = saving_process_command))    
  
  
  
  # Generating pdf output.
  pdf(paste("../Plots/part02_14_cross_correlation_distance_multidimensional_scaling_plots_clusters_mortality_c37_", method_current, graph_suffix, ".pdf", sep = ""), height = 52, width = 40)
  # Defining the number of plots
  par(mfrow = c(1, 1))
