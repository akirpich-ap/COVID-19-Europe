...
for( len_current in part12_01_inteval_aggregations )
{
  ...
  for (current_index in c(1:dim(pairs_frame_incidence_tw)[1])) {
    ...
    filling_tw_current <- paste0( "pairs_frame_incidence_tw$tw_statistic_len", len_current, "[current_index] <- ts_test_current_result$distance" ) 
  ...
  for (current_index in c(1:dim(pairs_frame_mortality_tw)[1])) {
    ...
    filling_tw_current <- paste0( "pairs_frame_mortality_tw$tw_statistic_len", len_current, "[current_index] <- ts_test_current_result$distance" ) 
    eval(parse(text = filling_tw_current))    
