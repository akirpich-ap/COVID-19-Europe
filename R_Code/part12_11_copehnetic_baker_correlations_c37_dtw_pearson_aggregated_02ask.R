...
vector_of_titles <- c("Population Health and Socio-Demographic Data",
                      ...
                      "COVID-19 Incidence (Dynamic Time Wrapping)",
                      "COVID-19 Mortality (Dynamic Time Wrapping)",
                      "COVID-19 Vaccinations (Dynamic Time Wrapping)",
...
for( len_current in part12_01_inteval_aggregations )
{
  ...
  for (method_current in hclust_methods)
  {
    ...
    # path
    dynamic_time_wapping_hclust_incidence_path <- paste0("../R_Data/dynamic_time_wapping_hclust_incidence_c37_", method_current, "_aggr_", len_current, ".RData") 
    dynamic_time_wapping_hclust_mortality_path <- paste0("../R_Data/dynamic_time_wapping_hclust_mortality_c37_", method_current, "_aggr_", len_current, ".RData") 
    # loading
    ...
    # dynamic_time_wapping_hclust
    # path
    dynamic_time_wapping_hclust_vaccination_path <- paste0("../R_Data/dynamic_time_wapping_hclust_vaccination_c37_", method_current, "_aggr_", len_current, ".RData") 
    # loading
    ...
    dynamic_time_wapping_hclust_incidence_dengrogram_text <- 
      paste0("dynamic_time_wapping_hclust_incidence_c37_dengrogram_current <- as.dendrogram(dynamic_time_wapping_hclust_incidence_c37_", method_current, "_aggr_", len_current, ")") 
    dynamic_time_wapping_hclust_mortality_dengrogram_text <- 
      paste0("dynamic_time_wapping_hclust_mortality_c37_dengrogram_current <- as.dendrogram(dynamic_time_wapping_hclust_mortality_c37_", method_current, "_aggr_", len_current, ")") 
    # running 
    ...
    dynamic_time_wapping_hclust_vaccination_dengrogram_text <- 
      paste0("dynamic_time_wapping_hclust_vaccination_c37_dengrogram_current <- as.dendrogram(dynamic_time_wapping_hclust_vaccination_c37_", method_current, "_aggr_", len_current, ")") 
    # running 
    ...
    for ( correlation_current in c(1:length(correlations_list)) )
    {
      ...
      # Saving object
      object_creation_for_current_correlation_and_aggr_code <- paste0("dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, " <- cor_dendlist_current") 
      eval(parse(text = object_creation_for_current_correlation_and_aggr_code))    
      
      # Path to save
      path_to_save_object <- paste0("../R_Data/dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ".RData") 
      # Saving process
      saving_process_command <- paste0( "save( dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ", file = path_to_save_object )" )
      eval(parse(text = saving_process_command))    

      
      
      # Exporting CSV files
      dtw_correlation_current_csv_path = paste0("../R_Output/dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ".csv")
      ...
      write.table(x = frame_text_summary_out, file = dtw_correlation_current_csv_path, sep = ",", row.names = FALSE, col.names = TRUE)
      ...
      # Exporting as tex file
      # Creating a path 
      frame_text_summary_out_xtable_path <- paste0("../R_Output/dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ".tex")
      # Printing
