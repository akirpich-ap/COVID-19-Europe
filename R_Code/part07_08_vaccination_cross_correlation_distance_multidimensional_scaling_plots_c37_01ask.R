...
# vaccination
cc_distance_matrix_vaccination_c37 = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(cc_distance_matrix_vaccination_c37) = countries
rownames(cc_distance_matrix_vaccination_c37) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    if (current_country1 != current_country2) {
      cc_distance_matrix_vaccination_c37[current_country1, current_country2] = pairs_frame_vaccination_cc$cc_statistic[row_index]
    }
...
cc_distance_matrix_vaccination_c37 <- cc_distance_matrix_vaccination_c37/max(cc_distance_matrix_vaccination_c37) * 1000
# Exporting 
cc_distance_matrix_vaccination_c37_csv_path = paste("../R_Output/cc_distance_matrix_vaccination_c37.csv", sep = "")
write.table(x    = data.frame( country = gsub(pattern = " ", replacement = ".", x = rownames(cc_distance_matrix_vaccination_c37)), cc_distance_matrix_vaccination_c37),
            file = cc_distance_matrix_vaccination_c37_csv_path, sep = ",", row.names = FALSE)




# vaccination
cc_distance_points_vaccination = cmdscale(cc_distance_matrix_vaccination_c37)
...
pdf(paste("../Plots/part07_06_vaccination_cross_correlation_distance_vaccination_c37", graph_suffix, ".pdf", sep = ""), height = 16, width = 16)
...
for (method_current in hclust_methods)
{
  ...
  # Computing clusters
  cluster_vaccination = hclustfunc(cc_distance_matrix_vaccination_c37, method_selected = method_current)
  
  
  # Fix 2022.11.28 
  
  # Creating object
  create_object_command <- paste0( "cross_correlation_distance_hclust_vaccination_c37_", method_current, " <- cluster_vaccination" )
  eval(parse(text = create_object_command))    
  
  # Saving object
  # Path to save
  path_to_save_object <- paste0("../R_Data/cross_correlation_distance_hclust_vaccination_c37_", method_current, ".RData") 
  # Saving process
  saving_process_command <- paste0( "save( cross_correlation_distance_hclust_vaccination_c37_", method_current, ", file = path_to_save_object )" )
  eval(parse(text = saving_process_command))    
  
  
  
  # Generating pdf output.
  pdf(paste("../Plots/part07_06_vaccination_cross_correlation_distance_clusters_vaccination_c37_", method_current, graph_suffix, ".pdf", sep = ""), height = 52, width = 40)
  # Defining the number of plots
  par(mfrow = c(1, 1))
