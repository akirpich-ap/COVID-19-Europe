...
for( len_current in part12_01_inteval_aggregations )
{
  ...
  # Current indexes
  # indexes_current_cv <- sort(part11_01_cross_validation_samples_to_keep[len_current,])
  ...
  for (method_current in hclust_methods)
  {
    ...
    # path
    cross_correlation_distance_hclust_incidence_path <- paste0("../R_Data/cross_correlation_distance_hclust_incidence_c37_", method_current, "_aggr_", len_current, ".RData") 
    cross_correlation_distance_hclust_mortality_path <- paste0("../R_Data/cross_correlation_distance_hclust_mortality_c37_", method_current, "_aggr_", len_current, ".RData") 
    # loading
    ...
    # cross_correlation_distance_hclust
    # path
    cross_correlation_distance_hclust_vaccination_path <- paste0("../R_Data/cross_correlation_distance_hclust_vaccination_c37_", method_current, "_aggr_", len_current, ".RData") 
    # loading
    ...
    cross_correlation_distance_hclust_incidence_dengrogram_text <- 
      paste0("cross_correlation_distance_hclust_incidence_c37_dengrogram_current <- as.dendrogram(cross_correlation_distance_hclust_incidence_c37_", method_current, "_aggr_", len_current, ")") 
    cross_correlation_distance_hclust_mortality_dengrogram_text <- 
      paste0("cross_correlation_distance_hclust_mortality_c37_dengrogram_current <- as.dendrogram(cross_correlation_distance_hclust_mortality_c37_", method_current, "_aggr_", len_current, ")") 
    # running 
    ...
    auxilliary_data_hclust_complete_dengrogram_text          <- 
      paste0("auxilliary_data_hclust_complete_c37_dengrogram_current <- as.dendrogram(auxilliary_data_hclust_complete_c37_", method_current, ")")
    
    auxilliary_data_hclust_demographics_dengrogram_text      <- 
      paste0("auxilliary_data_hclust_demographics_c37_dengrogram_current <- as.dendrogram(auxilliary_data_hclust_demographics_c37_", method_current, ")") 
    
    auxilliary_data_hclust_population_health_dengrogram_text <- 
      paste0("auxilliary_data_hclust_population_health_c37_dengrogram_current <- as.dendrogram(auxilliary_data_hclust_population_health_c37_", method_current, ")") 
    # run
    ...
    # cross_correlation_distance_hclust
    # text
    cross_correlation_distance_hclust_vaccination_dengrogram_text <- 
      paste0("cross_correlation_distance_hclust_vaccination_c37_dengrogram_current <- as.dendrogram(cross_correlation_distance_hclust_vaccination_c37_", method_current, "_aggr_", len_current, ")") 
    # running 
    ...
    strain_distrubution_distance_hclust_genetics_dengrogram_text <- 
      paste0("strain_distrubution_distance_hclust_genetics_c37_dengrogram_current <- as.dendrogram(strain_distrubution_distance_hclust_genetics_c37_", method_current, ")") 
    # running 
    ...
    for ( correlation_current in c(1:length(correlations_list)) )
    {
      ...
      # Saving object
      object_creation_for_current_correlation_and_aggr_code <- paste0("ccd_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, " <- cor_dendlist_current") 
      eval(parse(text = object_creation_for_current_correlation_and_aggr_code))    
      
      # Path to save
      path_to_save_object <- paste0("../R_Data/ccd_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ".RData") 
      # Saving process
      saving_process_command <- paste0( "save( ccd_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ", file = path_to_save_object )" )
      eval(parse(text = saving_process_command))    

      
      
      # Exporting CSV files
      ccd_correlation_current_csv_path = paste0("../R_Output/ccd_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ".csv")
      
      frame_text_summary_out <- data.frame(name = rownames(cor_dendlist_current), cor_dendlist_current, check.names = FALSE )
      # Fix 2025.06.24. Re-arranging rowns
      frame_text_summary_out <- frame_text_summary_out[sort(rownames(cor_dendlist_current)), c("name", sort(rownames(cor_dendlist_current))) ] 
      
      
      write.table(x = frame_text_summary_out, file = ccd_correlation_current_csv_path, sep = ",", row.names = FALSE, col.names = TRUE)
      ...
      # Exporting as tex file
      # Creating a path 
      frame_text_summary_out_xtable_path <- paste0("../R_Output/ccd_correlation_", correlations_list[correlation_current] , "_", method_current, "_aggr_", len_current, ".tex")
      # Printing
...
# End of -> for( len_current in c(1:dim(part11_01_cross_validation_samples_to_keep)[1]) )
}  
