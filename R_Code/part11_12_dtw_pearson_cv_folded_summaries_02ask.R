...
for (method_current in hclust_methods)
{
  ...
  for ( correlation_current in c(1:length(correlations_list)) )
  {
    ...
    for( cv_current_index in c(1:dim(part11_01_cross_validation_samples_to_keep)[1]) )
    {
      ...
      # Path to load
      path_to_load_object <- paste0("../R_Data/dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_fold_", cv_current_index, ".RData") 
      # Loading process
      load( file = path_to_load_object )
      
      command_to_add_matrix <- paste0("matrix_list_current[[", cv_current_index, "]] <- dtw_correlation_", correlations_list[correlation_current] , "_", method_current, "_fold_", cv_current_index) 
    ...
    command_to_save_with_correct_name <- paste0("matrix_list_dtw_correlation_", correlations_list[correlation_current] , "_", method_current, " <- matrix_list_current") 
...
for (method_current in hclust_methods)
{
  ...
  for ( correlation_current in c(1:length(correlations_list)) )
  {
    ...
    command_to_save_with_working_name <- paste0("matrix_list_current <- matrix_list_dtw_correlation_", correlations_list[correlation_current] , "_", method_current) 
    ...
    # Exporting CSV files
    cor_dendlist_current_csv_path = paste0("../R_Output/part11_11_dtw_pearson_cv_folded_summaries_c37_", method_current, "_", correlations_list[correlation_current], ".csv")
    ...
    # Exporting as tex file
    # Creating a path 
    frame_text_summary_out_xtable_path <- paste0("../R_Output/part11_11_dtw_pearson_cv_folded_summaries_c37_", method_current, "_", correlations_list[correlation_current], ".tex")
    # Printing
