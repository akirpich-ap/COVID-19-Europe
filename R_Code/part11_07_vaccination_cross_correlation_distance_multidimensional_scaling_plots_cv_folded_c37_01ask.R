...
# vaccination
cc_distance_matrix_vaccination_c37_all_countries = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(cc_distance_matrix_vaccination_c37_all_countries) = countries
rownames(cc_distance_matrix_vaccination_c37_all_countries) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    if (current_country1 != current_country2) {
      cc_distance_matrix_vaccination_c37_all_countries[current_country1, current_country2] = pairs_frame_vaccination_cc$cc_statistic[row_index]
    }
...
for( cv_current_index in c(1:dim(part11_01_cross_validation_samples_to_keep)[1]) )
{
  ...
  # Vaccination
  cc_distance_matrix_vaccination_c37 <- cc_distance_matrix_vaccination_c37_all_countries[indexes_current_cv, indexes_current_cv]
  dim(cc_distance_matrix_vaccination_c37)
  ...
  for (method_current in hclust_methods)
  {
    ...
    # Creating object
    create_object_command <- paste0( "cross_correlation_distance_hclust_vaccination_c37_", method_current, "_fold_", cv_current_index, " <- cluster_vaccination" )
    eval(parse(text = create_object_command))    
    
    # Saving object
    # Path to save
    path_to_save_object <- paste0("../R_Data/cross_correlation_distance_hclust_vaccination_c37_", method_current, "_fold_", cv_current_index, ".RData") 
    # Saving process
    saving_process_command <- paste0( "save( cross_correlation_distance_hclust_vaccination_c37_", method_current, "_fold_", cv_current_index, ", file = path_to_save_object )" )
