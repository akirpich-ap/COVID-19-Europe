...
for( aggr_current_index in c(1:length(part12_01_inteval_aggregations)) )
{
  ...
  for (current_country1 in countries)
  {
    for (current_country2 in countries)
    {
      ...
      command_to_extract <- paste0("cc_statistic_vaccination_current_aggr <- pairs_frame_vaccination_cc$cc_statistic_len", aggr_current_index ) 
      eval(parse(text = command_to_extract))    
      
      if (current_country1 != current_country2) {
        cc_distance_matrix_vaccination_c37[current_country1, current_country2] = cc_statistic_vaccination_current_aggr[row_index]
      }
  ...
  for (method_current in hclust_methods)
  {
    ...
    # Creating object
    create_object_command <- paste0( "cross_correlation_distance_hclust_vaccination_c37_", method_current, "_aggr_", aggr_current_index, " <- cluster_vaccination" )
    eval(parse(text = create_object_command))    
    
    # Saving object
    # Path to save
    path_to_save_object <- paste0("../R_Data/cross_correlation_distance_hclust_vaccination_c37_", method_current, "_aggr_", aggr_current_index, ".RData") 
    # Saving process
    saving_process_command <- paste0( "save( cross_correlation_distance_hclust_vaccination_c37_", method_current, "_aggr_", aggr_current_index, ", file = path_to_save_object )" )
