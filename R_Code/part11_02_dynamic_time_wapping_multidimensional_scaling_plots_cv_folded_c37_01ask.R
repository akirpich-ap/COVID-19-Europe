...
# incidence
tw_distance_matrix_incidence_c37_all_countries = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(tw_distance_matrix_incidence_c37_all_countries) = countries
rownames(tw_distance_matrix_incidence_c37_all_countries) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    if (current_country1 != current_country2) {
      tw_distance_matrix_incidence_c37_all_countries[current_country1, current_country2] = pairs_frame_incidence_tw$tw_statistic[row_index]
    }
...
# mortality
tw_distance_matrix_mortality_c37_all_countries = matrix(0, nrow = matrix_dim, ncol = matrix_dim)
colnames(tw_distance_matrix_mortality_c37_all_countries) = countries
rownames(tw_distance_matrix_mortality_c37_all_countries) = countries
# Filling the matrix
for (current_country1 in countries)
{
  for (current_country2 in countries)
  {
    ...
    # Filling the value
    if (current_country1 != current_country2) {
      tw_distance_matrix_mortality_c37_all_countries[current_country1, current_country2] = pairs_frame_mortality_tw$tw_statistic[row_index]
    }
...
for( cv_current_index in c(1:dim(part11_01_cross_validation_samples_to_keep)[1]) )
{
  ...
  # Incidence
  tw_distance_matrix_incidence_c37 <- tw_distance_matrix_incidence_c37_all_countries[indexes_current_cv, indexes_current_cv]
  dim(tw_distance_matrix_incidence_c37)
  # Mortality
  tw_distance_matrix_mortality_c37 <- tw_distance_matrix_mortality_c37_all_countries[indexes_current_cv, indexes_current_cv]
  dim(tw_distance_matrix_mortality_c37)
  ...
  for (method_current in hclust_methods)
  {
    ...
    # Creating object
    create_object_command <- paste0( "dynamic_time_wapping_hclust_incidence_c37_", method_current, "_fold_", cv_current_index, " <- cluster_incidence" )
    eval(parse(text = create_object_command))    
    
    # Saving object
    # Path to save
    path_to_save_object <- paste0("../R_Data/dynamic_time_wapping_hclust_incidence_c37_", method_current, "_fold_", cv_current_index, ".RData") 
    # Saving process
    saving_process_command <- paste0( "save( dynamic_time_wapping_hclust_incidence_c37_", method_current, "_fold_", cv_current_index, ", file = path_to_save_object )" )
  ...
  for (method_current in hclust_methods)
  {
    ...
    # Creating object
    create_object_command <- paste0( "dynamic_time_wapping_hclust_mortality_c37_", method_current, "_fold_", cv_current_index, " <- cluster_mortality" )
    eval(parse(text = create_object_command))    
    
    # Saving object
    # Path to save
    path_to_save_object <- paste0("../R_Data/dynamic_time_wapping_hclust_mortality_c37_", method_current, "_fold_", cv_current_index, ".RData") 
    # Saving process
    saving_process_command <- paste0( "save( dynamic_time_wapping_hclust_mortality_c37_", method_current, "_fold_", cv_current_index, ", file = path_to_save_object )" )
    eval(parse(text = saving_process_command))    
