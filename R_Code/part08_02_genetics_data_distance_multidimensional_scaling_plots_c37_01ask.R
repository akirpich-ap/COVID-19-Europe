...
genetics_data_matrix_c37 <- genetics_data_matrix_ordered_c37/max(genetics_data_matrix_ordered_c37) * 1000
# Exporting 
genetics_data_matrix_c37_csv_path = paste("../R_Output/genetics_data_matrix_c37.csv", sep = "")
write.table(x    = data.frame( country = gsub(pattern = " ", replacement = ".", x = rownames(genetics_data_matrix_c37)), genetics_data_matrix_c37),
            file = genetics_data_matrix_c37_csv_path, sep = ",", row.names = FALSE)
...
# genetics data
gd_distance_points_genetics = cmdscale(genetics_data_matrix_c37)
...
# Generating pdf output.
  pdf(paste("../Plots/part08_02_strain_distribution_distance_multidimensional_scaling_plots_c37", graph_suffix, ".pdf", sep = ""), height = 16, width = 16)
...
for (method_current in hclust_methods)
{
  ...
  # Computing clusters
  cluster_genetics = hclustfunc(genetics_data_matrix_c37, method_selected = method_current)
  
  
  # Fix 2022.11.28 
  
  # Creating object
  create_object_command <- paste0( "strain_distrubution_distance_hclust_genetics_c37_", method_current, " <- cluster_genetics" )
  eval(parse(text = create_object_command))    
  
  # Saving object
  # Path to save
  path_to_save_object <- paste0("../R_Data/strain_distrubution_distance_hclust_genetics_c37_", method_current, ".RData") 
  # Saving process
  saving_process_command <- paste0( "save( strain_distrubution_distance_hclust_genetics_c37_", method_current, ", file = path_to_save_object )" )
  eval(parse(text = saving_process_command))    

  

  # Generating pdf output.
  pdf(paste("../Plots/part08_02_genetic_data_distance_multidimensional_scaling_plots_clusters_genetics_c37_", method_current, graph_suffix, ".pdf", sep = ""), height = 52, width = 40)
  # Defining the number of plots
  par(mfrow = c(1, 1))
